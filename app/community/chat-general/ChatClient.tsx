...
export default function ChatClient({ channelId = 'general', onActiveUsersUpdate, onRoomCountsUpdate, scrollToMessageId }: ChatClientProps) {
  ...
  return (
    <div className="relative flex flex-col h-[70vh] md:w-3/4 border rounded-xl bg-white shadow p-3">
      {error && (
      ...
      {loading && messages.length === 0 && (
        ...
        {messages.map((m, i) => {
          ...
          {/* message bubble */}
            ...
            {m.senderName === senderName && (
              <img
                src={`https://graph.facebook.com/${session?.user?.id}/picture?type=square&height=80&width=80` || '/default-avatar.png'}
                alt="me"
                className="w-8 h-8 rounded-full"
              />
            )}
